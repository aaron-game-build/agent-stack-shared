...
| `KB_ROOT` | ue-py-init, ue-py-evolve, ue-doc-audit, ue-task-retrospective | Project KB module root (same key/semantics as the `rules` node's `KB_ROOT`; reused here for skills/commands) | `Docs/agent-knowledge` | `docs/ue-agent-knowledge` (draft) |
...
| `CONFIG_SCRIPT` | ue-python-config | Path (relative to project root) to the Python config script run via `-f` | `Content/Python/oath_project/validate_all_static.py`-adjacent `setup_gate0_map.py` (Oathboard's config entry is `Content/Python/setup_gate0_map.py`) | `Content/Python/final_phase1_config.py` (draft) |
