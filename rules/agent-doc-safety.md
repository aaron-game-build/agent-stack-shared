...
- On a 乱码 / encoding symptom, targeted-grep `{{KB_ROOT_FROM_PROJECT}}` before improvising a fix. CJK→CJK mojibake (e.g. `仇恨实验` → `浠囨仺瀹為獙`) is still valid UTF-8, so a naive "is it valid UTF-8?" check gives a false all-clear.
